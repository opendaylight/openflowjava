...
abstract class AbstractConnectionAdapter implements ConnectionAdapter {
    ...
    public Future<RpcResult<Void>> packetOut(final PacketOutInput input) {
        final SimpleRpcListener future = new SimpleRpcListener(input, "packet-out-input sending failed");
        final ChannelFuture channelFuture = channel.write(new MessageListenerWrapper(future.takeMessage(),
                future.takeListener()));
