...
final class StackedOutboundQueue implements OutboundQueue {
    ...
    private static final AtomicIntegerFieldUpdater<StackedOutboundQueue> RESERVE_OFFSET_UPDATER =
    AtomicIntegerFieldUpdater.newUpdater(StackedOutboundQueue.class, "reserveOffset");
    private volatile int reserveOffset = -1;
    ...
    @GuardedBy("unflushedSegments")
    private void ensureSegment(final StackedSegment segment, final int offset) {
        ...
        if (segmentOffset >= unflushedSegments.size()) {
            final StackedSegment newSegment = StackedSegment.create(segment.getBaseXid() + (StackedSegment.SEGMENT_SIZE * unflushedSegments.size()));
    ...
    public Long reserveEntry() {
        ...
        final int offset = RESERVE_OFFSET_UPDATER.incrementAndGet(this);
        if (offset >= StackedSegment.SEGMENT_SIZE) {
            LOG.debug("Queue {} falling back to slow reservation for offset {}", this, offset);

            // Multiple segments, this a slow path
            synchronized (unflushedSegments) {
                LOG.debug("Queue {} executing slow reservation for offset {}", this, offset);
                ...
                // Need to re-adjust offset for possible move of segments
                // before we acquired the lock
                final StackedSegment slowSegment = firstSegment;
                final int slowOffset = (int) (slowSegment.getBaseXid() - fastSegment.getBaseXid() + offset);
        ...
        final Long xid = fastSegment.getBaseXid() + offset;
    ...
    public void commitEntry(final Long xid, final OfHeader message, final FutureCallback<OfHeader> callback) {
        ...
        Preconditions.checkArgument(calcOffset >= 0, "Commit of XID {} does not match up with base XID {}", xid, fastSegment.getBaseXid());

        Verify.verify(calcOffset <= Integer.MAX_VALUE);
        final int fastOffset = (int) calcOffset;

        if (fastOffset >= StackedSegment.SEGMENT_SIZE) {
            LOG.debug("Queue {} falling back to slow commit at offset {}", this, fastOffset);
            ...
            LOG.debug("Queue {} slow commit completing at offset {} (segment {} offset {})", this, slowOffset, segment, segOffset);
            segment.commitEntry(segOffset, message, callback);
            LOG.debug("Queue {} slow commit completed at offset {} (segment {} offset {})", this, slowOffset, segment, segOffset);
        } else {
            fastSegment.commitEntry(fastOffset, message, callback);
        }
    ...
    /**
     * Called from commitEntry: we know there is a potential entry to be flushed,
     * so make sure we have a flush task which will take care of it.
     */
     ...
     * @param now
     * @return True if the queue has been fully written out.
     */
    boolean writeEntries(@Nonnull final Channel channel, final long now) {
        // Local cache
        ...
        while (channel.isWritable()) {
            ...
            if (flushOffset >= StackedSegment.SEGMENT_SIZE) {
                ...
                synchronized (unflushedSegments) {
                    ...
                    ensureSegment(firstSegment, flushOffset);
                    ...
                    // Allow reservations back on the fast path by decrementing reserveOffset
                    RESERVE_OFFSET_UPDATER.getAndAdd(this, - StackedSegment.SEGMENT_SIZE);

                    flushOffset = 0;
                    LOG.debug("Queue {} flush moved to segment {}", segment);
                }
            }
        }

        return flushOffset > reserveOffset;
    }

    Long reserveBarrierIfNeeded() {
        // FIXME: optimize barrier tracking
        return reserveEntry();
    }
    ...
    long startShutdown(final Channel channel) {
        ...
        synchronized (unflushedSegments) {
            ...
            final int offset = RESERVE_OFFSET_UPDATER.addAndGet(this, StackedSegment.SEGMENT_SIZE);
            shutdownOffset = offset - StackedSegment.SEGMENT_SIZE;
