...
public class MultipartReplyMessageFactory implements OFDeserializer<MultipartReplyMessage>,
    ...
    private MultipartReplyFlowCase setFlow(ByteBuf input) {
        ...
        while (input.readableBytes() > 0) {
            ...
            List<Instruction> instructions = DecodingUtils.deserializeInstructions(
                    subInput.readableBytes(), subInput, insDeserializer, false);
        ...
        while (tableFeaturesLength > 0) {
            ...
            if (type.equals(TableFeaturesPropType.OFPTFPTINSTRUCTIONS)
                ...
                HeaderDeserializer<Instruction> insDeserializer = registry.getDeserializer(new MessageCodeKey(
                        EncodeConstants.OF13_VERSION_ID, EncodeConstants.EMPTY_VALUE, Instruction.class));
                List<Instruction> instructions = DecodingUtils.deserializeInstructions(
                        propertyLength - COMMON_PROPERTY_LENGTH, input, insDeserializer, true);
                ...
                actionBuilder.setAction(DecodingUtils.deserializeActions(
                        propertyLength - COMMON_PROPERTY_LENGTH, input, deserializer, true));
                ...
                oxmBuilder.setMatchEntries(DecodingUtils.deserializeMatchEntries(
                        propertyLength - COMMON_PROPERTY_LENGTH, input, registry, true));
        ...
        while (input.readableBytes() > 0) {
            ...
            while (actualLength < bodyLength) {
                ...
                OFDeserializer<Action> deserializer = registry.getDeserializer(new MessageCodeKey(
                        EncodeConstants.OF13_VERSION_ID, EncodeConstants.EMPTY_VALUE, Action.class));
                List<Action> actionsList = DecodingUtils.deserializeActions(
                        bucketsLength - BUCKETS_HEADER_LENGTH, input, deserializer, false);
