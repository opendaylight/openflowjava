...
public class RoleRequestInputMessageFactory implements OFSerializer<RoleRequestInput> {
    ...
    @Override
    public void serialize(RoleRequestInput object, ByteBuf outBuffer) {
        ByteBufUtils.writeOFHeader(MESSAGE_TYPE, object, outBuffer, EncodeConstants.EMPTY_LENGTH);
        outBuffer.writeInt(object.getRole().getIntValue());
        ByteBufUtils.padBuffer(PADDING_IN_ROLE_REQUEST_MESSAGE, outBuffer);
        outBuffer.writeLong(object.getGenerationId().longValue());
