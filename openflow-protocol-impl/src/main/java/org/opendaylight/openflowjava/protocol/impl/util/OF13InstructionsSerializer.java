 ...
 */
public class OF13InstructionsSerializer implements HeaderSerializer<Instruction> {
    ...
    private void writeActionsInstruction(ByteBuf out,
        ...
        if (instruction.getAugmentation(ActionsInstruction.class) != null) {
            ...
            OFSerializer<Action> serializer = serializerTable.getSerializer(
    ...
    @Override
    public void serialize(Instruction object, ByteBuf outBuffer) {
        Class<? extends InstructionBase> type = object.getType();
        if (type.isAssignableFrom(GotoTable.class)) {
            writeTypeAndLength(outBuffer, GOTO_TABLE_TYPE, GOTO_TABLE_LENGTH);
            outBuffer.writeByte(object.getAugmentation(TableIdInstruction.class).getTableId());
            ...
            MetadataInstruction metadata = object.getAugmentation(MetadataInstruction.class);
            ...
            writeActionsInstruction(outBuffer, object, WRITE_ACTIONS_TYPE);
        } else if (type.isAssignableFrom(ApplyActions.class)) {
            writeActionsInstruction(outBuffer, object, APPLY_ACTIONS_TYPE);
            ...
            outBuffer.writeInt(object.getAugmentation(MeterIdInstruction.class).getMeterId().intValue());
        } else if (type.isAssignableFrom(Experimenter.class)) {
            ExperimenterInstruction experimenter = object.getAugmentation(ExperimenterInstruction.class);
            byte[] data = experimenter.getData();
            writeTypeAndLength(outBuffer, EXPERIMENTER_TYPE, EXPERIMENTER_LENGTH + data.length);
    ...
    @Override
    public void serializeHeader(Instruction object, ByteBuf outBuffer) {
        Class<? extends InstructionBase> type = object.getType();
        if (type.isAssignableFrom(GotoTable.class)) {
            ...
            ExperimenterInstruction experimenter = object.getAugmentation(ExperimenterInstruction.class);
            writeTypeAndLength(outBuffer, EXPERIMENTER_TYPE, EncodeConstants.EXPERIMENTER_IDS_LENGTH);
            outBuffer.writeInt(experimenter.getExperimenter().intValue());
        }
    }
