...
public class PortStatusMessageFactory implements OFDeserializer<PortStatusMessage> {
    ...
    public PortStatusMessage bufferToMessage(ByteBuf rawMessage, short version) {
        ...
        int bytesBeforeNull = rawMessage.bytesBefore((byte) 0);
        rawMessage.readBytes(portNameBytes);
        String portName = new String(portNameBytes,0,bytesBeforeNull);
        builder.setName(portName);
