 ...
 *
 */
public class OxmTunDstDeserializer extends AbstractOxmMatchEntryDeserializer
        implements OFDeserializer<MatchEntry> {

    @Override
    public MatchEntry deserialize(final ByteBuf input) {
        MatchEntryBuilder builder = processHeader(getOxmClass(), getOxmField(), input);
        addTunDstValue(input, builder);
        return builder.build();
    }

    private static void addTunDstValue(final ByteBuf input, final MatchEntryBuilder builder) {
        TunDstCaseBuilder caseBuilder = new TunDstCaseBuilder();
        TunDstBuilder ipv4Builder = new TunDstBuilder();
        ...
        caseBuilder.setTunDst(ipv4Builder.build());
