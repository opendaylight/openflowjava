...
public class NiciraExtensionsRegistrator implements AutoCloseable {
    ...
    public void registerNiciraExtensions() {
        for (SwitchConnectionProvider provider : providers) {
            provider.registerSerializer(ActionCodec.MESSAGE_TYPE_KEY, ACTION_CODEC);
            provider.registerDeserializer(ActionCodec.MESSAGE_CODE_KEY, ACTION_CODEC);
            provider.registerSerializer(MatchReg0Codec.MESSAGE_TYPE_KEY, MATCH_REG0_CODEC);
            provider.registerDeserializer(MatchReg0Codec.MESSAGE_CODE_KEY, MATCH_REG0_CODEC);
            provider.registerSerializer(MatchReg1Codec.MESSAGE_TYPE_KEY, MATCH_REG1_CODEC);
            provider.registerDeserializer(MatchReg1Codec.MESSAGE_CODE_KEY, MATCH_REG1_CODEC);
            provider.registerSerializer(MatchReg2Codec.MESSAGE_TYPE_KEY, MATCH_REG2_CODEC);
            provider.registerDeserializer(MatchReg2Codec.MESSAGE_CODE_KEY, MATCH_REG2_CODEC);
            provider.registerSerializer(MatchReg3Codec.MESSAGE_TYPE_KEY, MATCH_REG3_CODEC);
            provider.registerDeserializer(MatchReg3Codec.MESSAGE_CODE_KEY, MATCH_REG3_CODEC);
            provider.registerSerializer(MatchReg4Codec.MESSAGE_TYPE_KEY, MATCH_REG4_CODEC);
            provider.registerDeserializer(MatchReg4Codec.MESSAGE_CODE_KEY, MATCH_REG4_CODEC);
            provider.registerSerializer(MatchReg5Codec.MESSAGE_TYPE_KEY, MATCH_REG5_CODEC);
            provider.registerDeserializer(MatchReg5Codec.MESSAGE_CODE_KEY, MATCH_REG5_CODEC);
            provider.registerSerializer(MatchReg6Codec.MESSAGE_TYPE_KEY, MATCH_REG6_CODEC);
            provider.registerDeserializer(MatchReg6Codec.MESSAGE_CODE_KEY, MATCH_REG6_CODEC);
            provider.registerSerializer(MatchReg7Codec.MESSAGE_TYPE_KEY, MATCH_REG7_CODEC);
            provider.registerDeserializer(MatchReg7Codec.MESSAGE_CODE_KEY, MATCH_REG7_CODEC);
            provider.registerSerializer(MatchTunIdCodec.MESSAGE_TYPE_KEY, MATCH_TUN_ID_CODEC);
            provider.registerDeserializer(MatchTunIdCodec.MESSAGE_CODE_KEY, MATCH_TUN_ID_CODEC);
        }
    }

    public void unregisterExtensions() {
        // TODO MS: add unregistration when it is supported in
