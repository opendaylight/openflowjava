 ...
 *
 */
public class OF13MultipartReplyExperimenterDeserializerTest {

    /**
     * Testing of {@link OF13MultipartReplyExperimenterDeserializer} for correct translation into POJO
     * @throws Exception 
     */
    @Test
    public void test() {
        ByteBuf buffer = UnpooledByteBufAllocator.DEFAULT.buffer();
        buffer.writeInt(456654);
        buffer.writeInt(888222);
        byte[] data = new byte[]{0, 0, 1, 1, 2, 2, 3, 3};
        buffer.writeBytes(data);

        OF13MultipartReplyExperimenterDeserializer deserializer =
                new OF13MultipartReplyExperimenterDeserializer();
        MultipartReplyExperimenter message = deserializer.deserialize(buffer);

        Assert.assertEquals("Wrong experimenter", 456654, message.getExperimenter().intValue());
        Assert.assertEquals("Wrong exp-type", 888222, message.getExpType().intValue());
