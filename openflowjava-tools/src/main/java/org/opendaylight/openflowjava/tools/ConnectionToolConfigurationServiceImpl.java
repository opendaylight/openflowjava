...
public class ConnectionToolConfigurationServiceImpl implements ConnectionToolConfigurationService {
    ...
    public void marshallData(ConnectionTestTool.Params params, String configurationName) throws JAXBException, SAXException {
        File file = new File(xmlFilePathWithFileName);
        LOG.info("Marshaling configuration data to: {}", xmlFilePathWithFileName);

        SchemaFactory sf = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        Schema schema = sf.newSchema(new File(xsdSchemaPathWithFileName));
        LOG.info("with schema: {}", xsdSchemaPathWithFileName);

        JAXBContext jaxbContext = JAXBContext.newInstance(Configurations.class);
        Marshaller jaxbMarshaller = jaxbContext.createMarshaller();

        jaxbMarshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
        jaxbMarshaller.setProperty(Marshaller.JAXB_NO_NAMESPACE_SCHEMA_LOCATION, xsdFileName);
    ...
    public ConnectionTestTool.Params unMarshallData(String configurationName) throws SAXException, JAXBException {
        SchemaFactory sf = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        Schema schema = sf.newSchema(new File(xsdSchemaPathWithFileName));
        LOG.debug("Loading schema from: {}", xsdSchemaPathWithFileName);
        ...
        Configurations configurations = (Configurations) unmarshaller.unmarshal(new File(xmlFilePathWithFileName));
        LOG.debug("Configurations ({}) are un-marshaled from {}", configurations.getConfiguration().size(), xmlFilePathWithFileName);
