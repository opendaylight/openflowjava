...
public class NxResubmitActionRegistrator implements AutoCloseable {
        ...
        if (providers != null) {
            for (SwitchConnectionProvider provider : providers) {
                ...
                provider.registerSerializer(new EnhancedMessageTypeKey<>(EncodeConstants.OF13_VERSION_ID,
                        Action.class, NxActionResubmit.class), new NxActionResubmitSerializer());
            }
        }
    }
    ...
    public void registerNxResubmitDeserializer(List<SwitchConnectionProvider> providers) {
        if (providers != null) {
            for (SwitchConnectionProvider provider : providers) {
                System.out.println("registering deserializer");
                provider.registerDeserializer(new MessageCodeKey(EncodeConstants.OF13_VERSION_ID,
                        EncodeConstants.EXPERIMENTER_VALUE, Action.class), new NxActionResubmitDeserializer());
            }
        }
    }
